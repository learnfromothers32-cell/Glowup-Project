...
export default function StylistSignupPage() {
  ...
  const alreadyAuthRef = useRef(isAuthenticated && user?.role === "stylist");

  useEffect(() => {
    if (alreadyAuthRef.current) {
      navigate("/stylist/onboarding", { replace: true });
    }
  }, [navigate]);
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="w-full max-w-sm space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button variant="secondary" onClick={async () => {
              try {
                await socialLogin("google", "stylist");
                navigate("/stylist/onboarding", { replace: true });
            ...
            <Button variant="secondary" onClick={async () => {
              try {
                await socialLogin("apple", "stylist");
                navigate("/stylist/onboarding", { replace: true });
