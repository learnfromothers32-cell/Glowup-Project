...
const BENEFITS = [
  { icon: TrendingUp, label: "Grow your client base", sub: "Access thousands of local clients looking for your services" },
  { icon: Users, label: "Smart scheduling", sub: "Manage bookings, availability, and queue in real-time" },
  { icon: Star, label: "Build your reputation", sub: "Showcase your portfolio and collect verified reviews" },
  { icon: Shield, label: "Secure payments", sub: "Get paid on time, every time, with payment protection" },
];

export default function StylistSignupPage() {
  const { register, socialLogin, isLoading, isAuthenticated, user } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    if (isAuthenticated && user?.role === "stylist") {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await register(name.trim(), email.trim(), password, "stylist");
      setSuccess(`Account created! Check ${email.trim()} to verify your email.`);
      setTimeout(() => navigate("/stylist/onboarding", { replace: true }), 2000);
    } catch (err: any) {
        ...
        <div className="relative z-10 max-w-lg">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <div className="flex items-center gap-2 mb-8">
              ...
              <div>
                <span className="text-white font-bold text-xl tracking-tight">GlowUp for Stylists</span>
                <p className="text-brand-300 text-xs">Grow your beauty business</p>
              </div>
            </div>
            <h1 className="text-4xl font-bold text-white leading-tight mb-4">Turn your passion into profit</h1>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="mt-10 pt-8 border-t border-white/10">
            <div className="flex items-center gap-4">
              ...
              <div className="text-xs text-brand-300">
                <span className="text-white font-semibold">500+</span> stylists already earning
              </div>
              <Badge variant="success" dot>Free to join</Badge>
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm space-y-6">
          <div className="text-center lg:text-left">
            ...
            <h1 className="text-h3 font-display text-text-primary">List your services</h1>
            <p className="text-body-sm text-text-secondary mt-1">Start growing your beauty business today</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="name" className="block text-body-sm font-semibold text-text-primary mb-1.5">Salon or full name</label>
              <input id="name" type="text" required value={name} onChange={(e) => setName(e.target.value)} className="input-field" placeholder="Your salon or personal name" />
            </div>
            <div>
              <label htmlFor="phone" className="block text-body-sm font-semibold text-text-primary mb-1.5">Phone number <span className="text-text-muted">(optional)</span></label>
              <input id="phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="input-field" placeholder="+233 XX XXX XXXX" />
            </div>
            <div>
            ...
            <Button type="submit" disabled={isLoading || !allValid} loading={isLoading} className="w-full">
              <Store size={14} /> Create stylist account
            </Button>
          </form>

          <p className="text-center text-caption text-text-muted">
            You'll complete your profile (services, portfolio, availability) during onboarding.
          </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button variant="secondary" disabled={isLoading} onClick={async () => {
              try {
                const u = await socialLogin("google", "stylist");
            ...
            <Button variant="secondary" disabled={isLoading} onClick={async () => {
              try {
                const u = await socialLogin("apple", "stylist");
          ...
          <p className="text-center text-caption text-text-muted">
            ...
            <Link to="/signup" className="font-semibold text-brand-500 hover:text-brand-600">Create a client account</Link>
          </p>
        </motion.div>
      </div>
    </div>
