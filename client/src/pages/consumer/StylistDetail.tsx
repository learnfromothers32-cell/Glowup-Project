    ...
    <span className="inline-flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <Star key={n} size={size}
          className={n <= Math.round(rating) ? "text-amber-400" : "text-slate-200 dark:text-slate-600"}
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3">
        ...
        <div className="flex gap-1.5">
          {images.map((_, i) => (
            <button key={i} onClick={() => setIndex(i)} className={`h-0.5 rounded-full transition-all duration-300 ${i === index ? "w-5 bg-blue-500" : "w-[5px] bg-white/20"}`} />
          ))}
        </div>
      </div>
    </motion.div>
...
function EmptyState({ icon: Icon, title, sub }: { icon: React.ElementType; title: string; sub: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4 bg-slate-100 dark:bg-slate-800">
        <Icon size={24} className="text-slate-400 dark:text-slate-500" />
      </div>
      <p className="text-sm font-semibold mb-1.5 text-slate-900 dark:text-slate-100">{title}</p>
      <p className="text-xs max-w-[240px] leading-relaxed text-slate-500 dark:text-slate-400">{sub}</p>
    </div>
  );
}
...
function PortfolioCarousel({ items, onView }: {
  ...
  return (
    <div className="relative group/carousel">
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
        {items.map((item, i) => (
          <motion.button key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            onClick={() => onView(i)}
            className="snap-start shrink-0 w-[260px] sm:w-[280px] rounded-2xl overflow-hidden cursor-pointer bg-white dark:bg-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.08)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.12)] transition-all duration-300 hover:scale-[1.02]">
            ...
            <div className="p-3.5">
              <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 truncate">{item.service || item.caption || `Work #${i + 1}`}</p>
              <div className="flex items-center justify-between mt-2">
                <span className="text-xs text-slate-500 dark:text-slate-400">{item.caption || `Transformation ${i + 1}`}</span>
      ...
      {canScrollLeft && (
        <button onClick={() => scrollTo("left")} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 w-9 h-9 rounded-full items-center justify-center bg-white dark:bg-slate-700 shadow-md text-slate-600 dark:text-slate-300 transition-all duration-200 hidden sm:flex hover:bg-slate-50 dark:hover:bg-slate-600 z-10" aria-label="Previous"><ChevronLeft size={16} /></button>
      )}
      {canScrollRight && (
        <button onClick={() => scrollTo("right")} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 w-9 h-9 rounded-full items-center justify-center bg-white dark:bg-slate-700 shadow-md text-slate-600 dark:text-slate-300 transition-all duration-200 hidden sm:flex hover:bg-slate-50 dark:hover:bg-slate-600 z-10" aria-label="Next"><ChevronRight size={16} /></button>
      )}
      {total > 1 && (
        <div className="flex items-center justify-center gap-1.5 mt-4">
          {items.map((_, i) => (
            <button key={i} onClick={() => { const el = scrollRef.current; if (!el) return; const cardWidth = el.children[0] ? (el.children[0] as HTMLElement).offsetWidth + 16 : 280; el.scrollTo({ left: i * cardWidth, behavior: "smooth" }); }}
              className={`rounded-full transition-all duration-300 ${i === currentIndex ? "w-6 h-1.5 bg-blue-500" : "w-1.5 h-1.5 bg-slate-300 dark:bg-slate-600"}`} aria-label={`Go to slide ${i + 1}`} />
          ))}
        </div>
      )}
    </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="flex flex-col items-center py-4 rounded-xl bg-white dark:bg-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
        ...
        {[{ key: "posts" as const, label: "Posts", count: images.length }, { key: "transforms" as const, label: "Transforms", count: beforeAfter.length }]
          ...
          .map(({ key, label, count }) => (
            <button key={key} onClick={() => setSubTab(key)}
              className={`relative pb-3 text-sm font-semibold transition-colors ${subTab === key ? "text-blue-600 dark:text-blue-400" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300"}`}>
              {label}<span className="ml-1.5 text-xs">({count})</span>
              {subTab === key && <motion.div layoutId="portfolioSub" className="absolute bottom-0 inset-x-0 h-0.5 rounded-full bg-blue-500" transition={{ type: "spring", stiffness: 420, damping: 36 }} />}
            </button>
          ))}
      </div>
      {subTab === "posts" ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
            {beforeAfter.map((item, i) => (
              <motion.button key={i} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.03 }}
                onClick={() => onViewTransform(i)} className="group relative aspect-square overflow-hidden cursor-pointer bg-slate-100 dark:bg-slate-800">
                {item.mediaType === "video" ? (
                  <>
      ...
      {subTab === "transforms" && beforeAfter.length > 0 && (
        <div className="text-center">
          <button onClick={() => navigate("/app/trending")} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold transition-all hover:scale-105 active:scale-95 bg-blue-500 text-white">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
          {cats.map((c) => (
            <button key={c} onClick={() => setCat(c)}
              className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${cat === c ? "bg-blue-500 text-white shadow-sm" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700"}`}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-3">
        {services.filter((s) => (s.category || "General") === cat).map((svc, i) => (
          <motion.div key={`${cat}-${i}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
            <div className="group rounded-xl overflow-hidden bg-white dark:bg-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.08)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.12)] transition-all duration-300 cursor-pointer"
              onClick={() => navigate(`/app/stylist/${stylistId}/service/${svc._id || svc.name.toLowerCase().replace(/\s+/g, "-")}`)}>
              {svc.popular && <div className="h-0.5 bg-gradient-to-r from-amber-400 to-blue-500" />}
              <div className="p-4 flex items-center gap-4">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 bg-blue-50 dark:bg-blue-500/10"><Scissors size={17} className="text-blue-500" /></div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{svc.name}</span>
                    {svc.popular && <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wide bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400">Popular</span>}
                  </div>
                  {svc.duration && <span className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400"><Clock size={10} /> {svc.duration}</span>}
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-lg font-bold tabular-nums text-slate-900 dark:text-slate-100">{svc.price || "\u2014"}</span>
                  <button onClick={(e) => { e.stopPropagation(); navigate(`/app/stylist/${stylistId}/service/${svc._id || svc.name.toLowerCase().replace(/\s+/g, "-")}`); }}
                    className="px-4 py-2 rounded-lg text-xs font-bold bg-blue-500 text-white hover:bg-blue-600 transition-all duration-200 active:scale-[0.97]">Book</button>
                </div>
              </div>
            </div>
          </motion.div>
...
function ReviewsTab({ stylist }: { stylist: ExtendedStylist }) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl overflow-hidden bg-white dark:bg-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
        <div className="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 dark:divide-slate-700">
          <div className="p-6 flex items-center gap-5">
            <div className="text-center shrink-0">
              <p className="text-5xl font-bold text-slate-900 dark:text-slate-100">{stylist.rating.toFixed(1)}</p>
              <div className="mt-2"><StarRating rating={Math.round(stylist.rating)} size={14} /></div>
              <p className="text-[10px] font-semibold uppercase tracking-wider mt-1.5 text-slate-500 dark:text-slate-400">{reviews.length} reviews</p>
            </div>
            <div className="flex-1 space-y-2">
              {dist.map((d) => (
                <div key={d.stars} className="flex items-center gap-2">
                  <span className="text-[11px] font-bold w-2.5 tabular-nums text-slate-500 dark:text-slate-400">{d.stars}</span>
                  <div className="flex-1 h-1.5 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-700">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${d.pct}%` }} transition={{ duration: 0.9, delay: 0.2, ease: "easeOut" }} className="h-full rounded-full bg-blue-500" />
                  </div>
                  <span className="text-[11px] w-3.5 tabular-nums text-right text-slate-500 dark:text-slate-400">{d.count}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="p-6">
            <p className="text-[10px] font-bold uppercase tracking-widest mb-4 text-slate-500 dark:text-slate-400">Category Scores</p>
            <div className="space-y-3.5">
              {[{ label: "Cleanliness", score: 4.9 }, { label: "Punctuality", score: 4.8 }, { label: "Skill Level", score: 5.0 }, { label: "Value", score: 4.7 }].map(({ label, score }) => (
                <div key={label} className="flex items-center gap-3">
                  <span className="text-xs w-20 shrink-0 text-slate-600 dark:text-slate-400">{label}</span>
                  <div className="flex-1 h-1.5 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-700">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${(score / 5) * 100}%` }} transition={{ duration: 0.9, delay: 0.3, ease: "easeOut" }} className="h-full rounded-full bg-blue-600" />
                  </div>
                  <span className="text-xs font-bold w-5 tabular-nums text-slate-900 dark:text-slate-100">{score}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-3">
          {reviews.map((rev, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="rounded-xl bg-white dark:bg-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.08)] p-5">
              <div className="flex items-start gap-3.5">
                <Avatar name={rev.user} size="sm" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1.5">
                    <div><p className="text-sm font-bold text-slate-900 dark:text-slate-100">{rev.user}</p><StarRating rating={rev.rating} size={11} /></div>
                    <span className="text-[10px] px-2.5 py-1 rounded-full font-medium bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400">{rev.date}</span>
                  </div>
                  <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">{rev.comment}</p>
                  <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-100 dark:border-slate-700">
                    <button className="flex items-center gap-1.5 text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-blue-500 transition-colors"><ThumbsUp size={11} /> Helpful</button>
                    <button className="flex items-center gap-1.5 text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-blue-500 transition-colors"><MessageSquare size={11} /> Reply</button>
                  </div>
                </div>
              </div>
            </motion.div>
...
function BookingCard({
  ...
  return (
    <div className="rounded-2xl overflow-hidden bg-white dark:bg-slate-800 shadow-lg">
      <div className="px-5 pt-5 pb-4 flex items-center gap-3 border-b border-slate-100 dark:border-slate-700">
        <div className="relative shrink-0">
          <div className="w-12 h-12 rounded-xl overflow-hidden shadow-inner">
            {stylist.image ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-base font-bold bg-gradient-to-br from-blue-500 to-blue-600 text-white">{getInitials(stylist.name)}</div>
            )}
          </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            <p className="text-sm font-bold truncate text-slate-900 dark:text-slate-100">{stylist.name}</p>
            {stylist.isVerified && <BadgeCheck size={14} className="shrink-0 text-blue-500" />}
            <FollowButton stylistId={stylist.id} isFollowing={stylist.isFollowing ?? false} onFollowChange={onFollow ?? (() => {})} />
          </div>
          <p className="text-xs flex items-center gap-1 mt-0.5 text-slate-500 dark:text-slate-400"><MapPin size={10} /> {getLocationString(stylist.location)}</p>
        </div>
        <div className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-blue-50 dark:bg-blue-500/10">
          <Star size={12} fill="currentColor" className="text-blue-500" />
          <span className="text-xs font-bold tabular-nums text-slate-900 dark:text-slate-100">{stylist.rating.toFixed(1)}</span>
          <span className="text-[10px] text-slate-500 dark:text-slate-400">({stylist.reviews.length})</span>
        </div>
      </div>
      <div className="px-5 py-5">
        <div className="flex items-start justify-between gap-2 flex-wrap">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1 text-slate-500 dark:text-slate-400">Starting from</p>
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl sm:text-4xl font-bold tabular-nums leading-none text-slate-900 dark:text-slate-100">{minPrice > 0 ? `GH\u20B5 ${minPrice}` : "\u2014"}</span>
              {minPrice > 0 && <span className="text-xs font-medium text-slate-500 dark:text-slate-400">/ session</span>}
            </div>
          </div>
          <div className="flex flex-col items-end gap-1.5 mt-0.5">
            ...
            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400">Usually responds in &lt;1hr</span>
          </div>
        </div>
      </div>
      <div className="mx-5 mb-5 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-700/50">
        <div className="grid grid-cols-3 divide-x divide-slate-100 dark:divide-slate-700">
          {[
            ...
            <div key={label} className="flex flex-col items-center py-3.5 gap-0.5">
              <SIcon size={13} className="text-blue-500" />
              <p className="text-base font-bold tabular-nums leading-tight text-slate-900 dark:text-slate-100">{value}</p>
      ...
      <div className="px-5 space-y-2.5">
        <button onClick={() => onBook()}
          className="w-full bg-blue-500 text-white hover:bg-blue-600 group relative overflow-hidden flex items-center justify-center gap-2.5 py-3.5 rounded-xl text-sm font-bold transition-all duration-300 active:scale-[0.98] shadow-[0_4px_20px_rgba(37,99,235,0.28)]">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={onMessage}
            className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 bg-slate-50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-500/10 dark:hover:text-blue-400">
        ...
        <button onClick={onJoinWaitlist} disabled={joiningWaitlist}
          className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 ${waitlistJoined ? "bg-green-50 dark:bg-green-500/10 text-green-600" : "bg-blue-50 dark:bg-blue-500/10 text-blue-600"}`}>
      ...
      <div className="mx-5 mt-4 mb-5 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-700/50">
        {[
          ...
          <div key={label} className={`flex items-center gap-3 px-4 py-3 ${idx < arr.length - 1 ? "border-b border-slate-100 dark:border-slate-700" : ""}`}>
            ...
            <span className="text-[11px] font-medium leading-snug text-slate-600 dark:text-slate-300">{label}</span>
          </div>
        ))}
      </div>
      <div className="px-5 pb-4 flex items-center justify-center gap-1.5">
        <Wifi size={11} className="text-slate-400" />
        <p className="text-[10px] text-slate-400">Real-time availability \u00B7 No hidden fees</p>
      </div>
    </div>
  );
}
...
function ConnectCard({ stylist }: { stylist?: any }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-md bg-white dark:bg-slate-800">
      <div className="px-4 py-3.5 flex items-center gap-2 border-b border-slate-100 dark:border-slate-700">
        <div className="w-0.5 h-4 rounded-full bg-blue-500" />
        <h3 className="text-xs font-bold text-slate-900 dark:text-slate-100">Find on Social</h3>
      </div>
      <div className="p-3 space-y-2">
        {socialLinks.map(({ icon: SIcon, label, sub, url, color, bg }) => (
          <a key={label} href={url} target="_blank" rel="noopener noreferrer"
            className="w-full flex items-center gap-3 px-3.5 py-3 rounded-xl text-left transition-all duration-200 no-underline bg-slate-50 dark:bg-slate-700/50 hover:bg-[var(--hover-bg)]"
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] truncate text-slate-500 dark:text-slate-400">{sub}</p>
            </div>
...
export default function StylistDetail() {
  ...
  if (!stylist) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-5 bg-slate-50 dark:bg-slate-900">
      <EmptyState icon={Users} title="Stylist not found" sub="This profile doesn't exist or has been removed." />
      <button onClick={() => navigate("/app")} className="px-5 py-2.5 rounded-xl text-sm font-bold bg-blue-500 text-white hover:bg-blue-600 transition-all">Browse Stylists</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="relative overflow-hidden" style={{ height: "clamp(360px, 58vh, 580px)" }}>
        {stylist.image ? (
          ...
          <div className="lg:hidden absolute inset-0 bg-gradient-to-br from-blue-500 to-blue-600" />
        )}
        ...
        <div className="hidden lg:flex absolute inset-0 flex-col items-center justify-center px-6 bg-gradient-to-br from-blue-500 to-blue-600">
          {stylist.image && (
            <>
              <img src={stylist.image} alt="" className="absolute inset-0 w-full h-full object-cover object-center opacity-10 blur-3xl" />
              <div className="absolute inset-0 bg-gradient-to-br from-blue-500/50 to-blue-600/50" />
            </>
          )}
          <div className="relative z-10 flex flex-col items-center gap-4 text-center max-w-2xl mx-auto">
            {stylist.image ? (
              ...
              <div className="w-32 h-32 rounded-full ring-4 ring-white/20 flex items-center justify-center text-4xl font-bold bg-gradient-to-br from-blue-600 to-blue-500 text-white">{getInitials(stylist.name)}</div>
            )}
            <div>
        ...
        <div className="lg:hidden absolute bottom-0 inset-x-0 px-4 sm:px-6 lg:px-8 pb-6 sm:pb-7">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {stylist.isVerified && (
                <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] sm:text-xs font-medium bg-blue-500/20 backdrop-blur-md text-blue-300">
      ...
      <div className="sticky top-0 z-40 bg-white/95 dark:bg-slate-800/95 backdrop-blur-2xl shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
            ...
            {tabs.map(({ key, label, icon: Icon, count }) => {
              ...
              return (
                <button key={key} onClick={() => goTo(key)}
                  className={`relative flex items-center gap-1.5 px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all duration-200 ${active ? "text-blue-600 bg-blue-50 dark:bg-blue-500/10" : "text-slate-500 dark:text-slate-400 bg-transparent"}`}>
                  <Icon size={13} />
                  <span>{label}</span>
                  {!!count && count > 0 && (
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${active ? "bg-blue-500 text-white" : "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400"}`}>{count}</span>
                  )}
                  {active && <motion.div layoutId="tabInd" className="absolute bottom-0 left-3 right-3 h-0.5 rounded-full bg-blue-500" transition={{ type: "spring", stiffness: 420, damping: 36 }} />}
                </button>
              );
            })}
          </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.16 }}>
                {activeTab === "about" && (
                  <div className="space-y-7">
                    <div className="flex flex-wrap gap-2">
                      {[
                        { icon: Shield, label: "Verified Professional", cls: "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400" },
                        { icon: Award, label: "Top Rated", cls: "bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400" },
                        { icon: Zap, label: "Quick Response", cls: "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400" },
                    ...
                    {stylist.bio && (
                      <div className="rounded-2xl overflow-hidden bg-white dark:bg-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
                        <div className="px-5 sm:px-6 py-4 flex items-center gap-2.5 border-b border-slate-100 dark:border-slate-700">
                          <div className="w-0.5 h-5 rounded-full bg-blue-500" />
                          <h2 className="text-sm font-bold text-slate-900 dark:text-slate-100">About</h2>
                        </div>
                        <div className="px-5 sm:px-6 py-5">
                          <p className="text-sm leading-[1.8] text-slate-600 dark:text-slate-300">{stylist.bio}</p>
                        </div>
                      </div>
                    )}
                    {services.length > 0 && (
                      <div>
                        <div className="flex items-end justify-between gap-2 flex-wrap mb-5">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-blue-50 dark:bg-blue-500/10"><Scissors size={15} className="text-blue-500" /></div>
                            <div>
                              <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Featured Services</h2>
                              <p className="text-xs mt-0.5 font-medium text-slate-500 dark:text-slate-400">{services.length} services available</p>
                            </div>
                          </div>
                          <button onClick={() => goTo("services")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-colors">
                        ...
                        <div className="flex gap-3 overflow-x-auto pb-1 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
                          {services.slice(0, 6).map((svc, i) => (
                            <motion.div key={i} initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                              className="snap-start shrink-0 w-[200px] sm:w-[220px] rounded-2xl overflow-hidden cursor-pointer group transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 bg-white dark:bg-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.08)]"
                              onClick={() => navigate(`/app/stylist/${id}/service/${svc._id || svc.name.toLowerCase().replace(/\s+/g, "-")}`)}>
                              {svc.popular && <div className="h-0.5 bg-gradient-to-r from-amber-400 to-blue-500" />}
                              <div className="p-4">
                                <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-3.5 bg-blue-50 dark:bg-blue-500/10"><Scissors size={15} className="text-blue-500" /></div>
                                <p className="text-xs font-bold mb-1 truncate text-slate-900 dark:text-slate-100">{svc.name}</p>
                                {svc.duration && <p className="text-[11px] flex items-center gap-1 mb-3.5 text-slate-500 dark:text-slate-400"><Clock size={9} /> {svc.duration}</p>}
                                <div className="flex items-center justify-between mt-3">
                                  <span className="text-base font-bold tabular-nums text-slate-900 dark:text-slate-100">{svc.price || "\u2014"}</span>
                                  <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-blue-500 text-white">Book</span>
                                </div>
                              </div>
                            </motion.div>
                          ))}
                    ...
                    {stylist.reviews.length > 0 && (
                      <div>
                        <div className="flex items-end justify-between gap-2 flex-wrap mb-5">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-blue-50 dark:bg-blue-500/10"><Star size={15} className="text-blue-500" /></div>
                            <div>
                              <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Client Reviews</h2>
                              <p className="text-xs mt-0.5 font-medium text-slate-500 dark:text-slate-400">{stylist.reviews.length} verified reviews</p>
                            </div>
                          </div>
                          <button onClick={() => goTo("reviews")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-colors">
                        ...
                        <div className="flex gap-3 overflow-x-auto pb-1 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
                          {stylist.reviews.slice(0, 5).map((rev: Review, i: number) => (
                            <motion.div key={i} initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                              className="snap-start shrink-0 w-[260px] sm:w-[280px] rounded-2xl p-4 bg-white dark:bg-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
                              <div className="flex items-center gap-2.5 mb-3">
                                <Avatar name={rev.user} size="sm" />
                                <div className="flex-1 min-w-0">
                                  <p className="text-xs font-bold truncate text-slate-900 dark:text-slate-100">{rev.user}</p>
                                  <StarRating rating={rev.rating} size={10} />
                                </div>
                                <span className="text-[9px] shrink-0 text-slate-500 dark:text-slate-400">{rev.date}</span>
                              </div>
                              <p className="text-xs leading-relaxed line-clamp-3 text-slate-600 dark:text-slate-300">{rev.comment}</p>
                            </motion.div>
                          ))}
                        </div>
                      </div>
                ...
                {activeTab === "products" && (
                  <div>
                    <div className="flex items-center gap-2 mb-5">
                      <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-blue-50 dark:bg-blue-500/10"><ShoppingBag size={15} className="text-blue-500" /></div>
                      <div>
                        <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Shop</h2>
                        <p className="text-xs mt-0.5 font-medium text-slate-500 dark:text-slate-400">Available products</p>
                      </div>
                    </div>
                    {products.length === 0 ? (
                      <p className="text-sm text-slate-500 dark:text-slate-400">No products available</p>
                    ) : (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {products.map((p: any) => (
                          <div key={p._id} className="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                            <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{p.name}</p>
                            {p.description && <p className="text-xs mt-1 text-slate-500 dark:text-slate-400">{p.description}</p>}
                            <p className="text-sm font-bold mt-2 text-blue-600">GH\u20B5 {p.price}</p>
                ...
                {activeTab === "packages" && (
                  <div>
                    <div className="flex items-center gap-2 mb-5">
                      <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-blue-50 dark:bg-blue-500/10"><Gift size={15} className="text-blue-500" /></div>
                      <div>
                        <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Packages</h2>
                        <p className="text-xs mt-0.5 font-medium text-slate-500 dark:text-slate-400">Service bundles</p>
                      </div>
                    </div>
                    {packages.length === 0 ? (
                      <p className="text-sm text-slate-500 dark:text-slate-400">No packages available</p>
                    ) : (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {packages.map((p: any) => (
                          <div key={p._id} className="p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                            <p className="text-base font-semibold text-slate-900 dark:text-slate-100">{p.name}</p>
                            {p.description && <p className="text-xs mt-1 text-slate-500 dark:text-slate-400">{p.description}</p>}
                            <div className="flex items-center gap-2 mt-3">
                              <p className="text-lg font-bold text-blue-600">GH\u20B5 {p.price}</p>
                              <span className="text-xs text-slate-500 dark:text-slate-400">\u2022 {p.totalSessions} sessions</span>
                            </div>
                            <p className="text-xs mt-1 text-slate-500 dark:text-slate-400">Expires in {p.expiryDays} days</p>
                            <button onClick={() => handleBuyPackage(p._id)} disabled={buyingPackage === p._id}
                              className="w-full mt-3 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all bg-blue-500 text-white hover:bg-blue-600">
                ...
                {activeTab === "memberships" && (
                  <div>
                    <div className="flex items-center gap-2 mb-5">
                      <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-blue-50 dark:bg-blue-500/10"><Crown size={15} className="text-blue-500" /></div>
                      <div>
                        <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Memberships</h2>
                        <p className="text-xs mt-0.5 font-medium text-slate-500 dark:text-slate-400">Subscription plans</p>
                      </div>
                    </div>
                    {tiers.length === 0 ? (
                      <p className="text-sm text-slate-500 dark:text-slate-400">No membership plans available</p>
                    ) : (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {tiers.map((t: any) => (
                          <div key={t._id} className="p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                            <p className="text-base font-semibold text-slate-900 dark:text-slate-100">{t.name}</p>
                            {t.description && <p className="text-xs mt-1 text-slate-500 dark:text-slate-400">{t.description}</p>}
                            <p className="text-lg font-bold mt-3 text-blue-600">GH\u20B5 {t.price}/{t.billingCycle}</p>
                            {t.benefits?.length > 0 && (
                              <ul className="mt-2 space-y-1">
                                {t.benefits.map((b: string, i: number) => (
                                  <li key={i} className="text-xs flex items-center gap-1.5 text-slate-600 dark:text-slate-300">
                                    <span className="text-green-600">\u2713</span> {b}
                                  </li>
                                ))}
                              </ul>
                            )}
                            {t.discountPercent > 0 && <p className="text-xs font-semibold mt-2 text-blue-600">{t.discountPercent}% discount on services</p>}
      ...
      <div className="fixed bottom-0 inset-x-0 z-50 lg:hidden">
        <div className="flex items-center gap-3 px-4 py-3.5 bg-white/96 dark:bg-slate-800/96 backdrop-blur-2xl shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
          <div className="flex-1">
            <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">From</p>
            <p className="text-xl font-bold tabular-nums leading-tight text-slate-900 dark:text-slate-100">{minPrice > 0 ? `GH\u20B5 ${minPrice}` : "Contact"}</p>
          </div>
          <button onClick={handleShare} className="w-11 h-11 rounded-xl flex items-center justify-center transition-all bg-slate-50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400">
            <Share2 size={17} />
          </button>
          <button onClick={handleMessage} className="w-11 h-11 rounded-xl flex items-center justify-center transition-all bg-slate-50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400">
            <MessageSquare size={17} />
          </button>
          <button onClick={() => handleBook()} className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all active:scale-[0.97] bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-[0_4px_16px_rgba(37,99,235,0.28)]">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: 20, x: "-50%" }} animate={{ opacity: 1, y: 0, x: "-50%" }} exit={{ opacity: 0, y: 20, x: "-50%" }}
            className="fixed bottom-28 lg:bottom-8 left-1/2 z-[400] flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl pointer-events-none bg-blue-500 text-white">
        ...
        {copied && (
          <motion.div initial={{ opacity: 0, y: 20, x: "-50%" }} animate={{ opacity: 1, y: 0, x: "-50%" }} exit={{ opacity: 0, y: 20, x: "-50%" }}
            className="fixed bottom-28 lg:bottom-8 left-1/2 z-[400] flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl pointer-events-none bg-blue-500 text-white">
            <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 bg-blue-400"><Check size={12} color="#fff" /></div>
