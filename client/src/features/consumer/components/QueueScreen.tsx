...
function QueueBadge({ position, size = "sm" }: { position: number; size?: "sm" | "lg" }) {
  ...
  return (
    <div className={`${dims} rounded-2xl flex flex-col items-center justify-center shrink-0 ${isNext ? "bg-gradient-to-br from-green-400 to-emerald-500 shadow-lg shadow-emerald-200" : "bg-gradient-to-br from-amber-400 to-orange-500 shadow-lg shadow-amber-200"}`}>
...
function StatusBadgeInline({ status }: { status: string }) {
  ...
  return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gray-100 text-gray-400 text-[10px] font-bold">{status}</span>;
}
...
function HeroCard({ booking, queueData, onView }: { booking: Booking; queueData?: QueueEntry | null; onView: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} onClick={onView}
      className="relative bg-white rounded-3xl border border-gray-100 overflow-hidden cursor-pointer hover:shadow-xl hover:shadow-gray-100/50 transition-all duration-300 mb-6 group">
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400 mt-0.5">{isNext ? "Get ready, you're up!" : isToday ? "Your appointment is today" : "Upcoming booking"}</p>
            </div>
          </div>
        ...
        <div className="flex gap-5">
          <div className="flex-1 min-w-0">
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight mb-0.5">{stylistName}</h2>
            <p className="text-sm text-gray-500 mb-3">{serviceName}</p>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span key={text} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-50 text-xs text-gray-500 font-medium">
          ...
          <div className="shrink-0 flex flex-col items-center gap-2 pt-1">
            {queueData ? (
              <>
                ...
                {isNext && <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider bg-emerald-50 px-2 py-1 rounded-lg">Go now</span>}
              </>
            ) : (
              <div className="w-16 h-16 rounded-2xl bg-gray-50 flex items-center justify-center text-gray-300"><Clock size={24} /></div>
            )}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-gradient-to-r from-gray-50 to-white border-t border-gray-100">
        <span className="text-xs text-gray-400 font-medium">Tap for full details</span>
...
function BookingRow({ booking, queueData, onView, onCancel, index }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }} onClick={onView}
      className={`group relative bg-white rounded-2xl border overflow-hidden cursor-pointer transition-all duration-200 ${isCancelled ? "border-red-100 opacity-60" : isToday ? "border-emerald-100 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-50" : "border-gray-100 hover:border-gray-200 hover:shadow-lg hover:shadow-gray-50"}`}>
      ...
      <div className="flex items-center gap-4 p-4 pl-5">
        <div className="shrink-0 w-14 text-center">
          <p className="text-lg font-black text-gray-900 tabular-nums leading-none">{fmtISO(booking.startTime).split(" ")[0]}</p>
          <p className="text-[10px] text-gray-400 font-semibold mt-0.5">{fmtISO(booking.startTime).split(" ")[1]}</p>
        </div>
        <div className="flex-1 min-w-0">
        ...
        <div className="shrink-0 flex flex-col items-end gap-2">
          ...
          {!isCancelled && (
            <button onClick={(e) => { e.stopPropagation(); onCancel(); }}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl border border-gray-200 text-[11px] font-semibold text-gray-400 hover:border-red-200 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all duration-200">
              <X size={10} /> Cancel
            </button>
          )}
        </div>
      ...
      <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">{stylistName}</h2>
              <p className="text-sm text-gray-400">{serviceName}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-9 h-9 rounded-xl flex items-center justify-center text-gray-400 hover:text-gray-900 hover:bg-gray-50 transition-colors">
            <X size={18} />
          </button>
        </div>
          ...
          <div className="px-6 py-3">
            <div className="flex items-center justify-between p-3 rounded-xl bg-white border border-gray-100">
              <div>
                ...
                <p className="text-xs font-mono text-gray-500">{booking._id.slice(0, 12)}…</p>
              </div>
              <button onClick={handleCopy} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 text-xs font-semibold text-gray-500 hover:bg-gray-50 transition-all">
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-gray-100 shrink-0 bg-gray-50/50">
          <button onClick={onClose} className="flex-1 py-3.5 rounded-2xl border border-gray-200 text-sm font-semibold text-gray-500 hover:bg-white transition-colors">Close</button>
          {!cancelled && (
      ...
      <div className="bg-gradient-to-b from-gray-50 to-white">
        <div className="max-w-3xl mx-auto px-5 pt-12 pb-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-1.5">My Schedule</p>
              <h1 className="text-3xl font-black text-gray-900 tracking-tight">Queue & Appointments</h1>
            </div>
            <button className="w-11 h-11 rounded-2xl bg-white border border-gray-200 flex items-center justify-center text-gray-400 hover:text-gray-700 hover:border-gray-300 hover:shadow-md transition-all">
              <Bell size={20} />
            </button>
          </div>
          ...
          {isLoading ? (
            ...
            <>
              <div className="grid grid-cols-3 gap-3 mb-6">
                {statCards.map((s) => (
                  <div key={s.label} className={`bg-white rounded-2xl border ${s.color.border || "border-gray-100"} p-4 hover:shadow-md transition-shadow`}>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
                ...
                <div className="relative shrink-0">
                  ...
                  <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search appointments…"
                    className="w-full sm:w-56 pl-10 pr-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-900 placeholder:text-gray-400 outline-none focus:border-gray-300 focus:bg-white focus:ring-2 focus:ring-gray-100 transition-all" />
                </div>
              </div>
                    ...
                    {grouped.map(([dateLabel, items]) => (
                      <div key={dateLabel} className="mb-6">
                        <div className="flex items-center gap-3 mb-4">
                          <h3 className="text-xs font-bold uppercase tracking-[0.15em] text-gray-400">{dateLabel}</h3>
                          <div className="flex-1 h-px bg-gradient-to-r from-gray-200 to-transparent" />
                          <span className="text-[10px] font-bold text-gray-300 bg-gray-50 px-2 py-0.5 rounded-full">{items.length}</span>
                        </div>
