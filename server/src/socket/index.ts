...
class RoomState {
  ...
  async has(room: string, member: string): Promise<boolean> {
    if (this.redis) {
      return (
        (await this.redis.sismember(`${this.prefix}:${room}`, member)) === 1
      );
    }
    this.lastAccessed.set(room, Date.now());
...
function registerRoomState(rs: RoomState) {
  roomStateInstances.push(rs);
}
...
export const initSocket = async (server: HttpServer): Promise<Server> => {
  ...
  liveNsp.on("connection", (socket: Socket) => {
    ...
    socket.on("live:join-room", async (data: { stylistId: string }) => {
      ...
      logger.info(
        `[LIVE-DEBUG] join-room: socket=${socket.id} user=${userId} role=${userRole} stylistId=${stylistId}`,
      );
      ...
      logger.info(
        `[LIVE-DEBUG] join-room: room=${currentRoom} members=[${roomMembers ? [...roomMembers].join(",") : "none"}]`,
      );
      ...
      if (userRole !== "stylist") {
        try {
          ...
          if (session) {
            if (session.viewerCount >= MAX_VIEWERS) {
              socket.emit("live:error", {
                message: "Stream is full. Try again later.",
              });
          ...
          logger.error("[live:join-room] error:", {
            error: (err as Error).message,
          });
        }
      }
      ...
      logger.info(
        `[LIVE-DEBUG] join-room: broadcasting user-joined to room=${currentRoom} viewerId=${viewerId} role=${userRole}`,
      );
      liveNsp
        .to(currentRoom)
      ...
      if (userRole === "stylist") {
        ...
        logger.info(
          `[LIVE-DEBUG] join-room: stylist joining, local room members=${memberCount}`,
        );
        if (roomMembers) {
          for (const memberId of roomMembers) {
            ...
            if (!memberSocket) {
              logger.info(
                `[LIVE-DEBUG] join-room: member ${memberId} NOT found in local sockets (likely on another instance)`,
              );
              continue;
            }
            ...
            if (memberUser?.role !== "stylist") {
              const memberUserId =
                memberUser?.id || `anon_${memberId.slice(-6)}`;
              logger.info(
                `[LIVE-DEBUG] join-room: sending user-joined to stylist for member=${memberId} userId=${memberUserId}`,
              );
              socket.emit("live:user-joined", {
                userId: memberUserId,
                userRole: memberUser?.role || "client",
                socketId: memberId,
              });
            }
          }
    ...
    socket.on("live:leave-room", async () => {
      ...
      if (userRole !== "stylist") {
        try {
          ...
          logger.error("[live:leave-room] error:", {
            error: (err as Error).message,
          });
        }
      }
        ...
        try {
          ...
          const userName =
            role === "stylist"
              ? "Stylist"
              : userId
                ? `Viewer_${userId.slice(-4)}`
                : `Guest_${socket.id.slice(-4)}`;
          ...
          logger.error("[live:send-message] error:", {
            error: (err as Error).message,
          });
        }
      },
    ...
    socket.on(
      "live:reaction",
      async (data: { stylistId: string; reaction: string }) => {
        const room = `live:${data.stylistId}`;
        ...
        try {
          ...
          logger.error("[live:reaction] error:", {
            error: (err as Error).message,
          });
        }
    ...
    socket.on(
      "live:pin-message",
      async (data: { stylistId: string; messageId: string }) => {
        if (userRole !== "stylist") return;
        const room = `live:${data.stylistId}`;
        try {
          await LiveChatMessage.findByIdAndUpdate(data.messageId, {
            isPinned: true,
          });
        } catch (err) {
          logger.error("[live:pin-message] error:", {
            error: (err as Error).message,
          });
        }
        liveNsp
          .to(room)
          .emit("live:message-pinned", {
    ...
    socket.on(
      ...
      async (data: { stylistId: string; messageId: string }) => {
        ...
        try {
          ...
          logger.error("[live:unpin-message] error:", {
            error: (err as Error).message,
          });
        }
        liveNsp
          .to(room)
          .emit("live:message-unpinned", {
            messageId: data.messageId,
            userId: userId || socket.id,
          });
      },
    ...
    socket.on(
      ...
      async (data: { stylistId: string; messageId: string }) => {
        ...
        try {
          ...
          logger.error("[live:remove-message] error:", {
            error: (err as Error).message,
          });
        }
        liveNsp
          .to(room)
          .emit("live:message-removed", {
            messageId: data.messageId,
            userId: userId || socket.id,
          });
      },
    ...
    socket.on(
      "live:mute-user",
      async (data: { stylistId: string; targetUserId: string }) => {
        if (userRole !== "stylist") return;
        const room = `live:${data.stylistId}`;
        await roomMutedUsers.add(room, data.targetUserId);
        liveNsp
          .to(room)
          .emit("live:user-muted", {
    ...
    socket.on(
      ...
      async (data: {
        ...
        if (!userId || userRole === "stylist") {
          socket.emit("live:error", {
            message: "Only authenticated viewers can send gifts.",
          });
          return;
        }

        if (!data.coinAmount || data.coinAmount <= 0 || data.coinAmount > 10000)
          return;
        ...
        try {
          ...
          logger.error("[live:send-gift] error:", {
            error: (err as Error).message,
          });
        }
      },
    ...
    socket.on("live:stream-end", async (data: { stylistId: string }) => {
      ...
      try {
        ...
        if (
          !stylist ||
          stylist.id.toString() !== session.stylistId.toString()
        ) {
          socket.emit("live:error", { message: "You do not own this stream." });
          return;
        }
      } catch (err) {
        logger.error("[live:stream-end] ownership check error:", {
          error: (err as Error).message,
        });
        return;
      }
    ...
    socket.on(
      "live:webrtc-offer",
      (data: { stylistId: string; offer: any; targetSocketId: string }) => {
        logger.info(
          `[LIVE-DEBUG] webrtc-offer: from=${socket.id} to=${data.targetSocketId}`,
        );
    ...
    socket.on(
      "live:webrtc-answer",
      (data: { stylistId: string; answer: any; targetSocketId: string }) => {
        logger.info(
          `[LIVE-DEBUG] webrtc-answer: from=${socket.id} to=${data.targetSocketId}`,
        );
    ...
    socket.on(
      ...
      (data: { stylistId: string; candidate: any; targetSocketId: string }) => {
    ...
    socket.on("live:request-stream", (data: { stylistId: string }) => {
      logger.info(
        `[LIVE-DEBUG] request-stream: from=${socket.id} room=${currentRoom}`,
      );
      const room = `live:${data.stylistId || currentRoom?.replace("live:", "")}`;
      if (!room) return;
      liveNsp.to(room).emit("live:request-stream", {
    ...
    socket.on("disconnect", async () => {
      if (currentRoom && userRole !== "stylist" && !hasLeftRoom) {
        ...
        try {
          ...
          logger.error("[live:disconnect] viewer decrement error:", {
            error: (err as Error).message,
          });
        }
        ...
        try {
          ...
          logger.error("[live:disconnect] stylist cleanup error:", {
            error: (err as Error).message,
          });
        }
      }
